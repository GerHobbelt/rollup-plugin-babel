...
var babelCore = require('@gerhobbelt/babel-core');
var babelHelperModuleImports = require('@gerhobbelt/babel-helper-module-imports');
...
function createPreflightCheck () {
	...
	return function ( ctx, options, dir ) {
		if ( !preflightCheckResults[ dir ] ) {
			...
			var check = babelCore.transform( inputCode, options ).code;

			if ( ~check.indexOf('class ') ) {
				options.plugins = (options.plugins || []).concat( fallbackClassTransform );
				check = babelCore.transform( inputCode, options ).code;
			}
	...
	return {
		pre: function pre (file) {
			...
			file.set('helperGenerator', function (name) {
				...
				return (cachedHelpers[name] = babelHelperModuleImports.addNamed(file.path, name, HELPERS));
			});
		},
	};
}
...
var unpackOptions = function (ref) {
	if ( ref === void 0 ) ref = {};
	var extensions = ref.extensions; if ( extensions === void 0 ) extensions = babelCore.DEFAULT_EXTENSIONS;
...
function babel ( options ) {
	...
	return {
		...
		load: function load ( id ) {
			...
			return babelCore.buildExternalHelpers( externalHelpersWhitelist, 'module' );
		},
